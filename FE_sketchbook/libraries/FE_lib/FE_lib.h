...
const byte IO_PORT1      = 14;
const byte IO_PORT2		 = 15;
const byte IO_PORT3      = 3;
const byte IO_PORT4      = 2;
...
unsigned long minTimeLapse           =   0;      // Time interval for recording data (milliseconds) 
													//Zero for first initial record, set to 100ms in startDataRecord()
// EEprom storage
...
unsigned int fMarks=0;          // Total forward marks
unsigned int bMarks=0;          // Total backward marks


const byte D14Index = 0;
const byte D15Index = 1;
const byte D3Index = 2;
const byte D2Index = 3;
const byte A0Index = 4;
const byte A1Index = 5;

bool Data2Record[6] = {0}; //[D14, D15, D3, D2, A0, A1]
...
int getSamples(byte pin, int numOfSamples, char type)
{
  ...
  if (type=='e') // Take current reading
  {
      Serial.print("Sample Sum: "); Serial.println(sampleAvg);
  Serial.print("Value Out: "); Serial.println(valueOut);
	// Post process current count
    valueOut = valueOut - cVolt1;
    valueOut = (valueOut < 1) ? 0 : valueOut;
  }
  
  analogReference(DEFAULT); // reset analog reference to DEFAULT (5.00 volts)
...
if (startRecording==1)
{
 ...
 theTime = timeNow();
  ...
  if((theTime - lastRecordedTime) < minTimeLapse)
  {
    return;
  }
...
#ifdef FE_ENCODERS_H //if we are using the encoder library
...
{ 
  ...
  Serial.println(total_bytes); 
    ...
    #ifdef FE_ENCODERS_H //if we are using the encoders library 
	  int nelements = 6; //6 elements auto recorded
	  Serial.println(1); //send if encoder data recorded
	#else
	  int nelements = 3; //else, 3 elements auto recorded
	  Serial.println(0); //send if encoder data not recorded
	#endif
...
for (int index=0; index < (sizeof(Data2Record)/sizeof(Data2Record[0])) ; index++) 
  {
	...
	nelements += Data2Record[index]; // counts number of optional elements recorded
  }
...
{
	recordData();
	if (digitalRead(buttonBypass)==LOW) return 1;
	else return 0;
...
void motorSpeed(byte motor, int percentSpeed)
{
  ...
  if (percentSpeed > 0){
    ...
    if (directionStatus != 1) {
      ...
      directionStatus = 1; // Set the AEV Direction Status to forward.
      ...
      if (directionStatus != 0) {
        ...
        directionStatus = 0; // Set the AEV Direction Status to reverse.
      ...
      if (previousSpeed < 0) {
        ...
        directionStatus = 0; // Set the AEV Direction Status remains reverse.

      }
      else if (previousSpeed > 0){

        directionStatus = 1; // Set the AEV Direction Status remains forward.
      }
    }
  // Determine which motor to apply power to.
  switch (motor) 
...
{
  if (sensor == IO_PORT1) Data2Record[D14Index] = 1; //set to record D14 data in EEPROM
  else if (sensor == IO_PORT2) Data2Record[D15Index] = 1; //set to record D15 data in EEPROM
  else if (sensor == IO_PORT3) Data2Record[D3Index] = 1; //set to record D3 data in EEPROM
  else if (sensor == IO_PORT4) Data2Record[D2Index] = 1; //set to record D4 data in EEPROM
}
//**************************************************************************
...
{
  if (sensor == IO_PORT1) Data2Record[A0Index] = 1; //set to record A0 data in EEPROM
  else if (sensor == IO_PORT2) Data2Record[A1Index] = 1; //set to record A1 data in EEPROM
}
//**************************************************************************
...
{
  startRecording=1;
  recordData(); //take initial data reading
  minTimeLapse = 100; //Set time interval for recording data
}
//**************************************************************************
...
void postProcess()
{
  ...
  // Collect data for an additional 1 seconds
  pauseFor(1);
  ...
  for (int k=0;k<= (sizeof(Data2Record)/sizeof(Data2Record[0])); k++)
  {
	  EEPROM.write(k+2,Data2Record[k]); delay(5);
	  }  
...
void EndMyCode()
{
	postProcess(); //run postProcess function for data collection;
}
#endif